...
class Resource extends OperatorResource implements Retrievable
{
    ...
    /** @var string */
    public $endedAt;
    protected $aliases = ['created_by_user_id' => 'createdByUserId', 'started_at' => 'startedAt', 'display_name' => 'displayName', 'revision_end' => 'revisionEnd', 'user_id' => 'userId', 'created_by_project_id' => 'createdByProjectId', 'image_ref' => 'imageRef', 'flavor_id' => 'flavorId', 'server_group' => 'serverGroup', 'original_resource_id' => 'originalResourceId', 'revision_start' => 'revisionStart', 'project_id' => 'projectId', 'ended_at' => 'endedAt'];
    ...
    {
        $response = $this->execute($this->api->getResourceMetric(), ['resourceId' => $this->id, 'metric' => $metric, 'type' => $this->type]);
    ...
    {
        $options = array_merge($options, ['resourceId' => $this->id, 'type' => $this->type]);
