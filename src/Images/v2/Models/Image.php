...
class Image extends OperatorResource implements Creatable, Listable, Retrievable, Deletable
{
    ...
    public $virtualSize;
    private $jsonSchema;
    protected $aliases = ['container_format' => 'containerFormat', 'disk_format' => 'diskFormat', 'min_disk' => 'minDisk', 'owner' => 'ownerId', 'min_ram' => 'minRam', 'virtual_size' => 'virtualSize'];
    /**
     * {@inheritdoc}
     */
    protected function getAliases()
    {
        return parent::getAliases() + ['created_at' => new Alias('createdAt', \DateTimeImmutable::class), 'updated_at' => new Alias('updatedAt', \DateTimeImmutable::class), 'fileUri' => new Alias('fileUri', \GuzzleHttp\Psr7\Uri::class), 'schemaUri' => new Alias('schemaUri', \GuzzleHttp\Psr7\Uri::class)];
    }
    ...
    public function update(array $data)
    {
        ...
        $data = (object) $data;
        $aliasNames = array_map(function (Alias $a) {
        ...
        $response = $this->execute($this->api->patchImage(), ['id' => $this->id, 'patchDoc' => $json, 'contentType' => 'application/openstack-images-v2.1-json-patch']);
        $this->populateFromResponse($response);
    }
    ...
    {
        $this->execute($this->api->postImageData(), ['id' => $this->id, 'data' => $stream, 'contentType' => 'application/octet-stream']);
    }
