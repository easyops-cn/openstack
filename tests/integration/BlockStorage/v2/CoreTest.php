...
class CoreTest extends TestCase
{
    ...
    public function volumes()
    {
        ...
        $replacements = ['{description}' => $this->randomStr(), "'{size}'" => 1, '{name}' => $this->randomStr(), '{volumeType}' => $volumeType->id, '{key1}' => $this->randomStr(), '{val1}' => $this->randomStr()];
    ...
    {
        $replacements = ['{name}' => $this->randomStr()];
        $this->logStep('Creating volume type');
    ...
    public function snapshots()
    {
        ...
        $replacements = ['{volumeId}' => $volume->id, '{name}' => $this->randomStr(), '{description}' => $this->randomStr()];
        ...
        $this->assertInstanceOf(Snapshot::class, $snapshot);
        $this->getService()->getSnapshot($snapshot->id)->mergeMetadata(['key1' => 'val1']);
