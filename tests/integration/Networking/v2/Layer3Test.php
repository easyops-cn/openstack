...
class Layer3Test extends TestCase
{
    ...
    {
        $network = $this->getV2Service()->createNetwork(['name' => $this->randomStr(), 'routerAccessible' => $routerAccessible]);
        $network->waitUntilActive();
        return $network;
    }
    private function createSubnet(Network $network, $cidr = '192.168.199.0/24')
    {
        return $this->getV2Service()->createSubnet(['networkId' => $network->id, 'name' => $this->randomStr(), 'ipVersion' => 4, 'cidr' => $cidr]);
    }
    ...
    public function floatingIps()
    {
        ...
        $router = $this->getService()->createRouter(['name' => $this->randomStr(), 'externalGatewayInfo' => ['networkId' => $externalNetwork->id, 'enableSnat' => true]]);
        $this->logStep('Create interface for subnet %subnet% and router %router%', ['%subnet%' => $subnet->id, '%router%' => $router->id]);
        ...
        $replacements = ['{networkId}' => $externalNetwork->id, '{portId}' => $port1->id, '{fixedIpAddress}' => $fixedIp];
