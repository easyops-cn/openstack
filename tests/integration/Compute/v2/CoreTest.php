...
class CoreTest extends TestCase
{
    ...
    {
        $this->network = $this->getNetworkService()->createNetwork(['name' => self::NETWORK, 'adminStateUp' => true]);
        $this->logStep('Created network {name} with id {id}', ['name' => $this->network->name, 'id' => $this->network->id]);
        $this->subnet = $this->getNetworkService()->createSubnet(['name' => self::SUBNET, 'networkId' => $this->network->id, 'ipVersion' => 4, 'cidr' => '10.20.30.0/24']);
        $this->logStep('Created subnet {name} with id {id}', ['name' => $this->subnet->name, 'id' => $this->subnet->id]);
        $this->volume = $this->getBlockStorageService()->createVolume(['name' => self::VOLUME, 'description' => '', 'size' => 1]);
    ...
    private function createServer()
    {
        ...
        $replacements = ['{serverName}' => $this->randomStr(), '{imageId}' => $this->imageId, '{flavorId}' => $flavorId, '{networkId}' => $this->network->id];
    ...
    {
        $name = $this->randomStr();
        $replacements = ['{serverId}' => $this->serverId, '{newName}' => $name];
        /** @var $server \OpenStack\Compute\v2\Models\Server */
    ...
    private function changeServerPassword()
    {
        ...
        $replacements = ['{serverId}' => $this->serverId, '{newPassword}' => $this->adminPass];
    ...
    private function resizeServer()
    {
        ...
        $replacements = ['{serverId}' => $this->serverId, '{flavorId}' => $resizeFlavorId];
        /** @var $server \OpenStack\Compute\v2\Models\Server */
    ...
    {
        $replacements = ['{serverId}' => $this->serverId, '{imageId}' => $this->imageId, '{adminPass}' => $this->adminPass];
        /** @var $server \OpenStack\Compute\v2\Models\Server */
    ...
    {
        $replacements = ['{flavorName}' => $this->randomStr()];
    ...
    {
        $name = $this->randomStr();
        $replacements = ['{serverId}' => $this->serverId, '{imageName}' => $name];
        /** @var $server \OpenStack\Compute\v2\Models\Server */
    ...
    {
        $replacements = ['{name}' => $this->keypairName];
    ...
    {
        $replacements = ['{serverId}' => $this->serverId, '{secGroupName}' => 'default'];
    ...
    {
        $replacements = ['{serverId}' => $this->serverId, '{volumeId}' => $this->volume->id];
        ...
        $this->logStep('Attached volume {volumeId} to server {serverId} with volume attachment id {volumeAttachmentId}', array_merge($replacements, ['{volumeAttachmentId}' => $volumeAttachment->id]));
    }
    ...
    {
        $replacements = ['{serverId}' => $this->serverId, '{networkId}' => $this->network->id];
