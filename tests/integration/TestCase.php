...
abstract class TestCase extends \PHPUnit\Framework\TestCase implements TestInterface
{
    ...
    {
        return sprintf("%s%s", $colorPrefix, $message) . "\33[0m\33[1;0m";
    }
    protected function logStep($message, array $context = [])
    {
        ...
        if ($duration >= 10) {
            $color = "\33[0m\33[1;31m";
            // red
        } elseif ($duration >= 2) {
            $color = "\33[0m\33[1;33m";
            // yellow
        } else {
            $color = "\33[0m\33[1;32m";
            // green
        }
    ...
    protected function deleteItems(\Generator $items)
    {
        foreach ($items as $item) {
            if ($item instanceof Deletable && property_exists($item, 'name') && strpos($item->name, $this->namePrefix) === 0) {
                $item->delete();
            }
        }
    }
