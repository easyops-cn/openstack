...
class KeypairTest extends TestCase
{
    ...
    {
        $opts = ['name' => self::KEYPAIR_NAME, 'publicKey' => 'ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAAAgQDx8nkQv/zgGgB4rMYmIf+6A4l6Rr+o/6lHBQdW5aYd44bd8JttDCE/F/pNRr0lRE+PiqSPO8nDPHw0010JeMH9gYgnnFlyY3/OcJ02RhIPyyxYpv9FhY+2YiUkpwFOcLImyrxEsYXpD/0d3ac30bNH6Sw9JD9UZHYcpSxsIbECHw=='];
        $expectedJson = \json_encode(['keypair' => ['name' => $opts['name'], 'public_key' => $opts['publicKey']]], JSON_UNESCAPED_SLASHES);
    ...
    public function test_it_retrieves()
    {
        ...
        $this->assertEquals('ssh-rsa AAAAAAABBBBBBBBBCCCCCCCCCCC foo@bar.com', $this->keypair->publicKey);
        $this->assertFalse($this->keypair->deleted);
    }
    public function test_it_retrieves_by_user_id()
    {
        $this->client->request('GET', 'os-keypairs/' . self::KEYPAIR_NAME, ['headers' => [], 'query' => ['user_id' => 'fake']])->shouldBeCalled()->willReturn($this->getFixture('keypair-get'));
