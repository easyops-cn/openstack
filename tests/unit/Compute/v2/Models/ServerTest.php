...
class ServerTest extends TestCase
{
    ...
    {
        $opts = ['name' => 'foo', 'imageId' => 'bar', 'flavorId' => 'baz'];
    ...
    {
        $opts = ['name' => 'foo', 'flavorId' => 'baz', 'blockDeviceMapping' => [['uuid' => 'aaaa-ddddd-bbbb-ccccc']]];
        $expectedJson = ['server' => ['name' => $opts['name'], 'flavorRef' => $opts['flavorId'], 'block_device_mapping_v2' => $opts['blockDeviceMapping']]];
        $this->setupMock('POST', 'servers', $expectedJson, [], 'server-post');
        $this->assertInstanceOf(Server::class, $this->server->create($opts));
    }
    /**
    ...
    {
        $this->server->create(['name' => 'some-server-name', 'flavorId' => 'apple']);
    }
    public function test_it_updates()
    {
        ...
        $expectedJson = ['server' => ['name' => 'foo', 'accessIPv4' => '0.0.0.0', 'accessIPv6' => '0:0:0:0:0:ffff:0:0']];
    ...
    {
        $userOptions = ['imageId' => 'newImage', 'name' => 'newName', 'metadata' => ['foo' => 'bar', 'baz' => 'bar'], 'personality' => [['path' => '/etc/banner.txt', 'contents' => base64_encode('Hi there!')]], 'adminPass' => 'foo'];
        $expectedJson = json_encode(['rebuild' => ['imageRef' => $userOptions['imageId'], 'name' => $userOptions['name'], 'metadata' => $userOptions['metadata'], 'personality' => $userOptions['personality'], 'adminPass' => $userOptions['adminPass']]], JSON_UNESCAPED_SLASHES);
    ...
    {
        $userOptions = ['imageId' => 'newImage', 'adminPass' => 'foo'];
        $expectedJson = ['rescue' => ['rescue_image_ref' => $userOptions['imageId'], 'adminPass' => $userOptions['adminPass']]];
    ...
    {
        $opt = ['name' => 'secgroup_to_remove'];
        $expectedJson = ['removeSecurityGroup' => $opt];
        $this->setupMock('POST', 'servers/serverId/action', $expectedJson, [], new Response(202));
        $this->server->removeSecurityGroup($opt);
    }
    public function test_it_add_security_group()
    {
        $opt = ['name' => 'secgroup_to_add'];
        $expectedJson = ['addSecurityGroup' => $opt];
        $this->setupMock('POST', 'servers/serverId/action', $expectedJson, [], new Response(202));
    ...
    public function test_it_attaches_volume()
    {
        ...
        $expectedJson = ['volumeAttachment' => ['volumeId' => $volumeId]];
    ...
    public function test_it_creates_interface_attachments()
    {
        ...
        $expectedJson = ['interfaceAttachment' => ['net_id' => $networkId]];
