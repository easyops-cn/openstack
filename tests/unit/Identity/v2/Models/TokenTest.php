...
class TokenTest extends TestCase
{
    ...
    {
        $this->token->populateFromArray(['issued_at' => date('c'), 'expires' => date('c', strtotime('tomorrow'))]);
        $this->assertFalse($this->token->hasExpired());
    }
    public function test_expiration_is_true_for_old_tokens()
    {
        $this->token->populateFromArray(['issued_at' => date('c', strtotime('last wednesday')), 'expires' => date('c', strtotime('last thursday'))]);
