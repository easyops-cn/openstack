...
class ServiceTest extends TestCase
{
    ...
    {
        $this->client->request('GET', 'v1/resource_type', ['headers' => []])->shouldBeCalled()->willReturn($this->getFixture('resourcetypes-get'));
    ...
    {
        $this->client->request('GET', 'v1/resource/generic', ['headers' => [], 'query' => ['limit' => 3]])->shouldBeCalled()->willReturn($this->getFixture('resources-get'));
    ...
    {
        $this->client->request('POST', 'v1/search/resource/generic', ['headers' => ['Content-Type' => 'application/json']])->shouldBeCalled()->willReturn($this->getFixture('resources-get'));
        $result = $this->service->searchResources(['type' => 'generic']);
        $this->assertContainsOnlyInstancesOf(Resource::class, $result);
    }
    public function test_it_search_resources_with_custom_type()
    {
        $this->client->request('POST', 'v1/search/resource/instance', ['headers' => ['Content-Type' => 'application/json']])->shouldBeCalled()->willReturn($this->getFixture('resources-get'));
        $result = $this->service->searchResources(['type' => 'instance']);
        $this->assertContainsOnlyInstancesOf(Resource::class, $result);
    }
    public function test_it_lists_metrics()
    {
        $this->client->request('GET', 'v1/metric', ['headers' => [], 'query' => ['limit' => 5]])->shouldBeCalled()->willReturn($this->getFixture('metrics-get'));
