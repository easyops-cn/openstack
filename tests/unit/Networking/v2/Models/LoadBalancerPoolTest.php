...
class LoadBalancerPoolTest extends TestCase
{
    ...
    {
        $opts = ['name' => 'pool1', 'description' => 'simple pool', 'protocol' => 'HTTP', 'lbAlgorithm' => 'ROUND_ROBIN', 'listenerId' => 'listener1', 'sessionPersistence' => ['type' => 'APP_COOKIE', 'cookie_name' => 'my_cookie'], 'adminStateUp' => true];
        $expectedJson = ['pool' => ['name' => $opts['name'], 'description' => $opts['description'], 'protocol' => $opts['protocol'], 'lb_algorithm' => $opts['lbAlgorithm'], 'listener_id' => $opts['listenerId'], 'session_persistence' => $opts['sessionPersistence'], 'admin_state_up' => $opts['adminStateUp']]];
    ...
    public function test_it_updates()
    {
        ...
        $this->pool->sessionPersistence = ['type' => 'APP_COOKIE', 'cookie_name' => 'new_cookie'];
        $this->pool->adminStateUp = false;
        $expectedJson = ['pool' => ['name' => 'foo', 'description' => 'bar', 'lb_algorithm' => 'LEAST_CONNECTIONS', 'session_persistence' => ['type' => 'APP_COOKIE', 'cookie_name' => 'new_cookie'], 'admin_state_up' => false]];
    ...
    {
        $opts = ['address' => '10.0.0.8', 'protocolPort' => 80, 'subnetId' => 'subnetId', 'adminStateUp' => true, 'weight' => 1];
        $expectedJson = ['member' => ['address' => $opts['address'], 'protocol_port' => $opts['protocolPort'], 'subnet_id' => $opts['subnetId'], 'admin_state_up' => $opts['adminStateUp'], 'weight' => $opts['weight']]];
    ...
    {
        $opts = ['adminStateUp' => false, 'tenantId' => 'tenantId', 'delay' => 1, 'type' => 'HTTP', 'expectedCodes' => '200', 'maxRetries' => 5, 'httpMethod' => 'GET', 'urlPath' => 'test', 'timeout' => 1];
        $expectedJson = ['healthmonitor' => ['admin_state_up' => $opts['adminStateUp'], 'tenant_id' => $opts['tenantId'], 'delay' => $opts['delay'], 'type' => $opts['type'], 'expected_codes' => $opts['expectedCodes'], 'max_retries' => $opts['maxRetries'], 'http_method' => $opts['httpMethod'], 'url_path' => $opts['urlPath'], 'timeout' => $opts['timeout'], 'pool_id' => $this->pool->id]];
